...
def get_evaluation_step_name(inference_steps: int, current_step: int) -> str:
    ...
    return "last" if is_last_step else f"step{current_step}"
...
class Amber(MeshGenerationAlgorithm):
    def __init__(self, algorithm_config: DictConfig, train_dataset: AmberDataset):
        ...
        self._total_training_steps: int = 25600
        self._current_training_step: int = 0
        self._cpr_enabled: bool = dataloader_config.add_strategy == "curriculum_prioritized"
    ...
    def on_train_epoch_end(self) -> None:
        if self._cpr_enabled:
            progress = min(1.0, self._current_training_step / max(1, self._total_training_steps))
            self.train_dataset.training_progress = progress
        ...
        if self._cpr_enabled:
            refreshed = self.train_dataset.maybe_refresh_priorities()
            if refreshed:
                self.log("online/priorities_refreshed", 1.0, on_epoch=True)
        ...
        *,
        ...
        for step in range(self.inference_steps):
            ...
            if inference_output.refinement_okay:
                ...
                step_name = get_evaluation_step_name(
                ...
                for remaining_step in range(step, self.inference_steps):
                    ...
                    remaining_step_name = get_evaluation_step_name(
                        inference_steps=self.inference_steps,
                        current_step=remaining_step,
                    )

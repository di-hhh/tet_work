...
class EdgeAwareGatBase(nn.Module, abc.ABC):
    ...
    def __init__(
        ...
        *,
        ...
        # 创建edge_aware_gat_stack
        self.edge_aware_gat_stack = EdgeAwareGatStack(stack_config=stack_config, latent_dimension=latent_dimension)

    def _get_assertions(self) -> Type[MessagePassingGraphAssertions]:
        return MessagePassingGraphAssertions

    @staticmethod
    def _get_input_embeddings() -> Type[MessagePassingInputEmbedding]: # 获取输入嵌入层
        return MessagePassingInputEmbedding

    @staticmethod
    def _get_message_passing_stack() -> Type[EdgeAwareGatStack]: # 获取消息传递栈
        return EdgeAwareGatStack

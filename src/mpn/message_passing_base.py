...
class MessagePassingBase(nn.Module, abc.ABC):
    ...
    def __init__(
        ...
        *,
        ...
        print(f"in_node_features:{in_node_features}") # 4
        print(f"in_edge_features:{in_edge_features}") # 2
        ...
        # [Claude Code] 修复：根据 stack_config 中的 stack_type 字段选择 Stack 或 UNet。
        # stack_type 为 "unet" 时使用 MessagePassingUNet，否则使用默认的 MessagePassingStack。
        # 这样 SupervisedMPN、get_mpn_from_graph 等上层代码无需任何修改。
        stack_type = stack_config.get("stack_type", "default") if stack_config else "default"
        if stack_type == "unet":
            from src.mpn.message_passing_unet import MessagePassingUNet
            self.message_passing_stack = MessagePassingUNet(
                latent_dimension=latent_dimension,
                stack_config=stack_config,
            )
        else:
            self.message_passing_stack = MessagePassingStack(

...
class ExpertGeometryDatasetPreparator(DatasetPreparator):
    ...
    def _prepare_source_and_mesh(self, data_idx: int, dataset_mode: str) -> Tuple[SourceData, MeshWrapper]:
        ...
        source_data = SourceData(
            ...
            dataset_name=self.task_config.name,
            data_point_path=data_point_path,
        )
        weighted_imitation_config = self.algorithm_config.get("weighted_imitation") or {}
        expert_mesh.source_data = source_data
        expert_mesh.weighted_imitation_config = weighted_imitation_config
        if weighted_imitation_config.get("enabled", False) and self.task_config.name in {"console", "mold"} and weighted_imitation_config.get("auto_prepare", False):
            ...
            ensure_console_mold_reference_cache(
                ...
                overwrite=bool(weighted_imitation_config.get("overwrite_cache", False)),
            )  # codex: prepare the Console/Mold physics cache during dataset construction instead of in each training step
